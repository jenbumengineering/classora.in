...
export async function GET(request: NextRequest) {
  try {
    ...
    let backups = await prisma.backup.findMany({
      orderBy: { createdAt: 'desc' }
    })

    // Check if we have backup files in filesystem that don't have database records
    ...
    if (fs.existsSync(backupDir)) {
      ...
      // Create records for files that don't have database records
      for (const file of backupFiles) {
        const existingRecord = backups.find(backup => 
          backup.name === file.name || backup.path === file.path
        )
        
        if (!existingRecord) {
          console.log(`Creating backup record for: ${file.name}`)
          
          const backupType = file.name.includes('pre-restore') ? 'automatic' : 'manual'
          const description = file.name.includes('pre-restore') 
